...
int main(void) {
    ...
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
            ...
            if (i == 0 || i == NX - 1 || j == 0 || j == NY - 1) {
                u[i][j] = 100.0;  // fixed boundary at 100°C
            }
        }
    }
    ...
    // OpenACC data region: keep u on the device, create u_new there
    #pragma acc data copy(u) create(u_new)
    for (int iter = 0; iter < MAX_ITER; iter++) {
        max_diff = 0.0;

        // Jacobi update on the GPU
        ...
        // Copy u_new back into u on the GPU
        ...
        if (max_diff < TOLERANCE) {
            printf("GPU (OpenACC) converged after %d iterations.\n", iter);
            break;
        }
    }
